...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public DefaultMQProducerImpl(final DefaultMQProducer defaultMQProducer, RPCHook rpcHook) {
        this.defaultMQProducer = defaultMQProducer; // 互相注册
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 注册当前 producer 到 MQClientInstance => group -> producer
                ...
                if (startFactory) {
                    mQClientFactory.start(); // 启动很多核心服务
                }
    ...
    private void checkConfig() throws MQClientException {
        Validators.checkGroup(this.defaultMQProducer.getProducerGroup()); // 校验生产者组的名字

        if (this.defaultMQProducer.getProducerGroup().equals(MixAll.DEFAULT_PRODUCER_GROUP)) { // 生产者组的名字不能和系统默认的相同 DEFAULT_PRODUCER
    ...
    private SendResult sendDefaultImpl(
        ...
        final SendCallback sendCallback, // 如果是同步，传null
        final long timeout
                    ...
                    try {
                        ...
                        switch (communicationMode) {
                            ...
                            case ONEWAY:
                                return null;
                            case SYNC:
                                if (sendResult.getSendStatus() != SendStatus.SEND_OK) { // 失败重试
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (null == topicPublishInfo || !topicPublishInfo.ok()) { // 找不到去 namesrv
            this.topicPublishInfoTable.putIfAbsent(topic, new TopicPublishInfo());
            this.mQClientFactory.updateTopicRouteInfoFromNameServer(topic);
            topicPublishInfo = this.topicPublishInfoTable.get(topic);
        }

        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) { // 找到了返回
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                if (requestHeader.getTopic().startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX)) { // 处理重试消息
